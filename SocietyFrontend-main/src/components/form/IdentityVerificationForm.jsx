...
const IdentityVerificationForm = ({ formData, handleChange }) => {
  ...
  const handleCancelPhoto = (fileField, previewField) => {
    handleIdentityFieldChange(fileField, null);
    handleIdentityFieldChange(previewField, null);
  };
    ...
    <Box sx={{ textAlign: "center" }}>
      ...
      {identityProofs[previewField] && (
        <Box sx={{ position: 'relative', display: 'inline-block', mt: 2 }}>
          ...
          {/* Cancel Button */}
          <IconButton
            onClick={() => handleCancelPhoto(fileField, previewField)}
            sx={{
              position: 'absolute',
              top: -8,
              right: -8,
